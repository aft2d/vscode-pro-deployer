...
export class SFTP extends Target implements TargetInterface {
    ...
    private ensureConnected(cb: Function, errorCb?: Function): void {
        if (this.isConnected && this.sftp) {
            cb();
            return;
        }

        Extension.appendLineToOutputChannel("[INFO][SFTP] Connection lost or not established, reconnecting...");
        this.connect(cb, errorCb);
    }
    ...
    upload(uri: vscode.Uri): Promise<vscode.Uri> {
        ...
        return new Promise<vscode.Uri>((resolve, reject) => {
            ...
            const job = <QueueTask>((cb) => {
                // Check connection before executing the job
                this.ensureConnected(
                    ...
                    (error: any) => {
                        Extension.appendLineToOutputChannel(
                            "[ERROR][SFTP] Failed to reconnect: " + error
                        );
                        cb(error);
                        reject(error);
                    }
